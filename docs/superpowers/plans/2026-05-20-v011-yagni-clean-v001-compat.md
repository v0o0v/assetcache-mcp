...
**Architecture:** TDD-역방향 (회귀 -20) 패턴 — 먼저 test 5 파일 삭제 → 회귀 기준선이 1103 → 1083 로 떨어지는 신호 확인 후, live 코드를 commit 단위로 부분 축소. dead module 본체는 의존성이 모두 제거된 뒤 마지막에 삭제 (import error 회피). i18n / rename fix / 문서 / version bump 는 그 뒤 별도 commit. 각 commit 후 `pytest -q` 회귀 1083 passed 유지.
...
**Target:** `1083 passed + 1 skipped + 40 deselected` (회귀 -20)
...
## Task 2: 마이그레이션 테스트 5 파일 일괄 삭제 (-20)
...
Expected: 합계 20 tests collected. 21 이 아니면 spec 의 `-20` 가정이 깨졌다는 신호 — 정지 후 사용자 확인.
...
- [ ] **Step 3: 회귀 확인 — 1083 신호**

```powershell
pytest -q
```

Expected: `1083 passed, 1 skipped, 40 deselected`. 21 이 정확히 빠지지 않으면 — 다른 파일이 마이그레이션 모듈을 import 해 ImportError 가 누적 fail 로 잡힘 → 출력 살펴 누락 파일 식별.
...
회귀 1103 → 1083 (-20). detect/runner/path_rewrite/router/cli_migrate
...
Expected: `1083 passed, 1 skipped, 40 deselected` (변화 없음 — 같은 test 함수가 parametrize 만 10 → 4 case 로 줄어듦. 모두 통과하므로 회귀 count 영향 0).
...
Expected: `1083 passed, 1 skipped, 40 deselected`. Task 2 의 cli_migrate 테스트는 이미 제거됐으니 이 시점에 import error 없음.
...
Expected: `1083 passed, 1 skipped, 40 deselected`.
...
Expected: `1083 passed, 1 skipped, 40 deselected`. ImportError 가 나오면 Step 1 의 grep 이 놓친 참조 → 출력 살펴 보강.
...
Expected: `1083 passed, 1 skipped, 40 deselected`. (tools 디렉터리는 test 에 포함 안 됨 — 영향 0)
...
§2 의 회귀 수치 1103 도 그대로 두기 — historical (M10 완료 시점). 또는 끝에 "v0.1.1 yagni-clean 후 baseline 1083" 짧은 노트 추가 (Task 15 에서 마지막에).
...
Expected: `1083 passed, 1 skipped, 40 deselected in <60s>`
...
**회귀 일관성** — 모든 task 의 expected `1083 passed, 1 skipped, 40 deselected` (Task 1 만 baseline `1103`). 도구 카운트 20 그대로 유지.
