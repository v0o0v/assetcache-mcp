...
```
game-asset-helper/             # 저장소 이름은 그대로 (GitHub URL 호환)
...
└── tests/
    ...
    ├── test_search.py
    ├── test_mcp_tools.py
    ├── test_migration_*.py    # M10 Phase 1
...
- **Phase 1** — 마이그레이션 helper: `migration/detect.py` + `migration/migrate.py` (copy/move + `.migrated_from_v001` idempotent 마커) + 웹 배너 + SSE 진행률 + CLI `--migrate=copy|move` + i18n msgid 10건
...
**Phase 4 (대기)** — `pyproject.toml` 최종 확정 + `assetcache-mcp` console_script entry point + `python -m build` (sdist + wheel) + TestPyPI 업로드 + 정식 PyPI 업로드 + GitHub Actions workflow.

**Phase 0~3 누적**: **1103 passed + 1 skipped + 40 deselected** (baseline 1046 + M10 +57). **MCP 20 도구 그대로**, 신규 의존성 0.
