...
자동 가능 항목 (Task 4.6, 2026-05-20 검증):

- 정식 PyPI 업로드: ✅ `twine upload --repository pypi dist\*` 성공 — https://pypi.org/project/assetcache-mcp/0.1.0/
- 별도 venv 에서 `pip install assetcache-mcp`: ✅ 의존성 100+ 정상 resolve + `assetcache.exe --version` → `assetcache-mcp 0.1.0` + `assetcache-mcp.exe` 콘솔 스크립트 존재 + module import 정상
- GitHub repo rename `v0o0v/game-asset-helper` → `v0o0v/assetcache-mcp` 완료 + `git remote set-url` 반영
- Trusted Publishing (OIDC) 셋업 완료 — https://pypi.org/manage/account/publishing/ 에 `assetcache-mcp` / `v0o0v` / `assetcache-mcp` / `publish.yml` 등록
- GitHub Actions publish workflow: ✅ `v0.1.0` tag push 시 자동 trigger + `skip-existing: true` 옵션으로 PyPI 의 기존 v0.1.0 silent skip + 29초만에 success ([run 26109058719](https://github.com/v0o0v/assetcache-mcp/actions/runs/26109058719))
- v0.1.0 GitHub release draft 생성 — 한국어 notes (설치 / 마이그레이션 / 변경 / 한계 / 자동 publish 안내)
...
### 시나리오 2 — 마이그레이션 copy (v0.0.1 → AssetCacheMCP) [v0.1.1 에서 dead code 제거]
...
### 시나리오 3 — 마이그레이션 move [v0.1.1 에서 dead code 제거]
...
### 시나리오 4 — PyPI 알림 (배너 + tray) [시나리오 2/3 다음 — 마이그레이션 관련]
...
### 시나리오 5 — CLI `--migrate=copy` [v0.1.1 에서 dead code 제거]
...
- [x] 정식 PyPI 업로드 (2026-05-20)

  ```powershell
  python -m twine upload --repository pypi --non-interactive --disable-progress-bar dist\*
  ```

  → 결과: ✅ `Uploading assetcache_mcp-0.1.0-py3-none-any.whl` + `Uploading assetcache_mcp-0.1.0.tar.gz` + `View at: https://pypi.org/project/assetcache-mcp/0.1.0/`

- [x] 정식 PyPI 설치 + console script 동작 (2026-05-20)

  ```powershell
  python -m venv $env:TEMP\pypi-smoke-venv
  & "$env:TEMP\pypi-smoke-venv\Scripts\pip.exe" install assetcache-mcp
  ```

  → 결과:
    - ✅ 의존성 100+ 정상 resolve (`torch 2.12.0`, `PySide6 6.11.1`, `mcp 1.27.1`, `fastapi 0.136.1`, `librosa 0.11.0`, `open_clip_torch 3.3.0`, `numpy 2.4.6`, `Pillow 12.2.0`, `pydantic 2.13.4`, ...)
    - ✅ `Successfully installed ... assetcache-mcp-0.1.0`
    - ✅ `assetcache.exe --version` → `assetcache-mcp 0.1.0`
    - ✅ `assetcache-mcp.exe` 콘솔 스크립트 파일 존재 (`main_mcp()` entry point — `--mcp` 자동 forwarding)
    - ✅ `import assetcache; assetcache.__version__` → `0.1.0`
    - ✅ `from assetcache.__main__ import main, main_mcp` 두 entry 함수 정상 import

  Claude Desktop config 의 `mcpServers` 에 `python -m assetcache --mcp` 또는 `assetcache-mcp` 콘솔 스크립트 사용 가능 (사용자 환경에서 별도 검증 권장).
...
**2026-05-20 후기**: M10 Phase 1 의 v0.0.1 (GameAssetHelper) 데이터 폴더 마이그레이션 helper 는 v0.1.1 (`chore/v011-yagni-clean`) 에서 yagni-clean 됐다. v0.0.1 외부 사용자·다운로드 0 확인 후, 관련 코드 + 테스트 21건 + i18n msgid 3건 + 문서 안내 일괄 제거. 시나리오 1~5 의 본문은 historical record 로 보존.
