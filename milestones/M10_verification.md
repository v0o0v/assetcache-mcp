...
### 1.3 Phase 4 빌드 / 설치 (대기)

Phase 4 진행 후 채울 항목:

- `python -m build` 결과 (sdist + wheel 생성 성공 여부)
- `pip install dist/assetcache_mcp-0.1.0-py3-none-any.whl` 후 `assetcache --version` 결과
- TestPyPI 업로드 + `pipx install --index-url https://test.pypi.org/simple/ assetcache-mcp` 결과
- 정식 PyPI 업로드 후 `pipx install assetcache-mcp` 결과
- GitHub Actions workflow tag push 자동 빌드 결과
...
- [ ] 버전 확인
  ...
  → 기대: `pyproject.toml` 의 version (Phase 4 까지는 `0.0.1+m10dev` 또는 `0.1.0-dev`).
