...
| [M4 — 검색 UX 풍부화](./M4_plan.md) | ✅ 완료 (머지 대기) | 100 + follow-up 19 (전체 452/452, `clip_integration` 2 + `mcp_integration` 2 옵트인 제외) | 수동 시나리오는 `M4_verification.md` §4 (GUI 풍부 UX 4 단계 — 좌 칩 패널 + 우 슬라이더/저장 패널 + label_query 입력 + 저장된 검색 라운드트립). **사용자 검증 보고로 4 페인 도출 → M5 spec** | label_query 파서·HybridSearcher 6채널 재배분·diversity·saved_searches 4 신규 MCP (12→16)·feedback_records signed weight 페널티·suggest_packs samples 풍부화·GUI 풍부 UX (M5 가 폐기 예정) |
| **M5 — 웹 GUI 전환 + 라이브러리 리디자인 + Claude pick** (5.5주) | **다음** (spec 작성됨) | — | — | FastAPI + HTMX + Alpine 로컬 웹서버 / 옵션 C 레이아웃 (상단 검색 + ⚙ 고급 + 우측 슬라이드 B/C/D 탭) / 와이드 카드 그리드 (썸네일 60×60 좌 + 텍스트 우, 사운드 인라인 ▶) / `request_user_pick` 신규 MCP 도구 (동기 long-poll 5분) / Qt 위젯 4개 + 테스트 2 파일 폐기. spec: [`docs/superpowers/specs/2026-05-17-m5-web-gui-and-library-redesign.md`](../docs/superpowers/specs/2026-05-17-m5-web-gui-and-library-redesign.md) |
| M6 — 시트 분석 + 애니메이션 (1주) | 대기 | — | — | 격자 자동 분할·Aseprite/TexturePacker JSON·suggest_animation_frames |
| M7 — Unity Asset Store 임포트 (1주) | 대기 | — | — | .unitypackage 파서·캐시 스캐너 |
| M8 — 패키징 + i18n (1주) | 대기 | — | — | PyInstaller/Tauri·gettext/Jinja i18n (Pack/프로젝트 탭 풍부 UX 도 여기로) |
