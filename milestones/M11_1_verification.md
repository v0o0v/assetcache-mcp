...
| ~~batch 경로는 sprite_meta / sound_meta (width/height/duration_ms 등) 채우지 않음 — FTS 는 labels + description + path 만으로 동작, 검색 정확도가 sync 경로 대비 약간 낮음~~ | ~~낮~~ | **✅ v0.2.x patch — `core/analyzer/tech_meta.py` 분리 + BatchPoller library_dir 주입으로 sprite_meta/sound_meta 충전. 파일 읽기 실패 시 graceful skip + labels/FTS 는 계속.** |
