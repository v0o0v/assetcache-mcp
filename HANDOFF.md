...
M10 (v2 — PyPI 1차 배포 + AssetCacheMCP rename) **완전 종료**. main `d9a3862` PR #11 + `7ba6551` PR #12 (workflow Node.js 24). **1103 passed + 1 skipped + 40 deselected**, MCP 20 도구. PyPI v0.1.0 publish 완료, Trusted Publishing (OIDC) 셋업 → 향후 tag push 한 줄로 자동 publish. GitHub repo rename `v0o0v/game-asset-helper` → `v0o0v/assetcache-mcp`. v0.1.1 (`chore/v011-yagni-clean`) 에서 v0.0.1 마이그레이션 helper (Phase 1 코드 + 테스트 + i18n) yagni-clean 완료.
...
| v0.1.1 yagni-clean | v0.0.1 마이그레이션 helper (Phase 1) 코드+테스트+i18n 일괄 제거 완료 (`chore/v011-yagni-clean`) | ✅ 완료 |
...
- ✅ v0.0.1 마이그레이션 helper (Phase 1) yagni-clean (`chore/v011-yagni-clean`)
